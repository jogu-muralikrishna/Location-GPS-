...
# ---------- Flask Routes (same as before, but using Supabase) ----------
@app.route('/')
def index():
    return render_template_string(HTML_TEMPLATE)  # same long HTML as in your original code
...
def admin():
    if request.method == 'POST':
        ...
        if password == 'admin123':
            ...
            html += '<td>' + ''.join(f'<th style="background:#ff6b6b; color:white; padding:8px;">{col}</th>' for col in columns) + '</tr>'
            for v in visitors:
                html += '<tr>'
                for col in columns:
                    val = v.get(col, '')
...
# ---------- Your long HTML_TEMPLATE goes here (unchanged) ----------
HTML_TEMPLATE = ''' ... '''  # Copy the exact HTML_TEMPLATE from your original code
